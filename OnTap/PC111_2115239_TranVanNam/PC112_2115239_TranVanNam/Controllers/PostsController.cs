...
using PagedList;
using PC112_2115239_TranVanNam.Models;

namespace PC112_2115239_TranVanNam.Controllers
{
    public class PostsController : Controller
    {
		...
		// GET: Posts
		public ActionResult Index(string searchString, string sortOrder, int? page, int? pageSize)
		{
			...
			// Luôn đảm bảo rằng kết quả được sắp xếp trước khi phân trang
			switch (sortOrder)
			{
				case "title_desc":
				...
				case "content_asc":
					...
					break;
				default:
					// Mặc định sẽ sắp xếp theo Title
					posts = posts.OrderBy(p => p.Title);
					break;
			}

			int pageNumber = page ?? 1;
			int defaultSize = pageSize ?? 5; // Giá trị mặc định nếu không có pageSize được cung cấp
			ViewBag.pageSize = new SelectList(new[] { "5", "10", "20", "50" }, defaultSize.ToString());

			// ToPagedList cần được gọi sau khi dữ liệu đã được sắp xếp
			return View(posts.ToPagedList(pageNumber, defaultSize));
		}
